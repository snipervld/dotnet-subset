...
internal sealed class CopySubset : SubsetCommandBase
{
    protected override IEnumerable<FileInfo> GetFilesToCopy(FileInfo projectOrSolution, DirectoryInfo rootFolder)
    {
        ...
        var nugetConfigFiles = GetNugetConfigFiles(rootFolder, projectsByFullPath); // remove if nuget.config is not needed
    ...
    private static IEnumerable<FileInfo> GetProjectFiles(Dictionary<string, Project> projects)
    {
        return projects.Values
            .SelectMany(project => new DirectoryInfo(project.DirectoryPath).EnumerateFiles("*.*", SearchOption.AllDirectories));
    }
}
